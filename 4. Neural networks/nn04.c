/*
    ...
    Version Notes:
        Compiles/Runs/Debugs with: gcc nn04.c -o nn04 -lm -O3 -fopenmp -fopt-info -pg && time ./nn04 && gprof ./nn04
        Inherits all settings of previous version if not stated otherwise
        Performance for 100.000 steps:
            No multithreading --------------------->  8.9 seconds
            CalculateInnerStates() multithreaded --> 17.1 seconds
            UpdateLayer() multithreaded -----------> 12.1 seconds
            Both multithreaded -------------------->  3.4 seconds
*/	
