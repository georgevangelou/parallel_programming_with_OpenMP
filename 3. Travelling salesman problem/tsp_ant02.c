...
Version Notes:
    Compiles/Runs/Debugs with: gcc tsp_ant02.c -o tsp_ant02 -lm -O3 -fopt-info -pg && time ./tsp_ant02 && gprof ./tsp_ant02    
	Inherits all settings of the previous version unless stated otherwise
	Important improvements over previous version:
	> Now each ant temporarily creates a boolean array that shows which cities it has visited
	> Now Tau values are stored to the power of ALPHA so that frequent use of pow() is avoided
	Needs:	~04 seconds to find optimal path for N=50 cities for ANTS=100 and REPETITIONS=20 with all optimizations listed below
				(2x_speedup from tsp_ant01)
*/
...
double TauValues_to_ALPHA[N][N]; // Pherormone values between all city pairs to the power of ALPHA 
...
void InitializeTauValues_2() {
	...
	for (int i=0; i<N; i++) {
		...
		for (int j=0; j<N; j++) {
			TauValues_to_ALPHA[i][j] = pow(TAU_INITIAL_VALUE, ALPHA); //(float) rand() / RAND_MAX;
		}
	}
	printf(" ===> Completed.\n");
}
