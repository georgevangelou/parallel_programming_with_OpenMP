...
Version Notes:
    Compiles/Runs/Debugs with: gcc tsp_hh03.c -o tsp_hh03 -lm -O3 -pg && time ./tsp_hh03 && gprof ./tsp_hh03
	...
	Next city in path is either the closest or second closest one, depending on the value of <PICK_CLOSEST_CITY_POSSIBILITY>
	Function IsInPath2() consumes the vast majority of the execution time
	Results when: 	PICK_CLOSEST_CITY_POSSIBILITY = 1.0 ===> Total path distance:  89517.46 (same results with previous version, as expected)
					PICK_CLOSEST_CITY_POSSIBILITY = 0.8 ===> Total path distance: 103822.38
	              	PICK_CLOSEST_CITY_POSSIBILITY = 0.6 ===> Total path distance: 115066.83		
	Needs:	~ 01 minutes 27 seconds to calculate the shortest steps path using 01 thread and all optimizations listed below 
*/
