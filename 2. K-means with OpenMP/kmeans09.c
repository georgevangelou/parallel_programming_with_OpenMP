...
Version Notes:
    Compiles with: gcc kmeans09.c -o kmeans09 -lm
    Inherits all settings of the previous version unless stated otherwise
    Uses indices to access array data
    Added new / Modified existing functionalities:
    --> Initially picking the first Nc vectors as centers instead of random ones
    --> Checking if a center already exists now takes vector values instead of indices into account
    --> Finding a new center if a current one has no members
    Executes the algorithm for 4 vectors of 2 dimensions and 2 classes and produces correct results
...
int main( int argc, const char* argv[] ) {
    ...
    } while( (diff > THRESHOLD) && (repetitions < 40) ) ;
