...
Version Notes:
    Compiles with: gcc kmeans13.c -o kmeans13 -lm -fopt-info -fopenmp -O3
    Inherits all settings of the previous version unless stated otherwise
    Minor differences compared to kmeans12 (experimental changes to reach optimal settings)
    Executes the algorithm for 100000 vectors of 1000 dimensions and 100 classes and produces correct results
    Needs ~5 seconds to reach 16 repetitions with all optimizations and schedule(static)
...
Profiler Output:
    ...
    91.81     13.36    13.36                             frame_dummy
    4.54     14.02     0.66                             estimateCenters
    3.71     14.56     0.54                             SetVec
...
float estimateClasses() {
    ...
    #pragma omp parallel for reduction(+:tot_min_distances) schedule(static)
    for (int w=0; w<N; w++) {
        float min_dist = 1e30;
		int temp_class = -1;
...
int main( int argc, const char* argv[] ) {
    ...
    do {
        ...
        printf("DISTANCE IMPROVEMENT: %.6f \n", diff);
